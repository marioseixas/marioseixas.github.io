...
def generate_mermaid_er_diagram(
    ...
    def add_relationship(
        ...
        label: str = "",
        from_data: Dict[str, Any] = None,  # Add from_data parameter
        to_data: Dict[str, Any] = None,  # Add to_data parameter
    ) -> None:
        """Adds a relationship between two entities."""
        safe_from = add_entity(from_entity, from_data or {})  # Pass from_data
        safe_to = add_entity(to_entity, to_data or {})  # Pass to_data
    ...
    def process_tag(tag_name: str, data: Dict[str, Any]) -> None:
        ...
        if ">" in tag_name:
            ...
            for part in parts:
                add_relationship(part, combined_tag, "}|--|{", "part of", data=tag_data.get(part))  # Pass data

        # Add hierarchical relationships
        for child in data.get("children", []):
            add_relationship(tag_name, child, "||--|{", "parent of", from_data=data, to_data=tag_data.get(child))  # Pass data

        # Add non-hierarchical relationships
        for related in data.get("related", []):
            add_relationship(tag_name, related, "||..||", "related to", from_data=data, to_data=tag_data.get(related))  # Pass data
